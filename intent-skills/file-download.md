...
- 下载状态：`downloads({since})`（guid/url/filename/state/bytes）；落盘：`downloadPath(guid, timeoutS)` 等 completed 给路径；落盘目录内建状态目录 downloads/（捕获随 tab 入口自动开，无需自设下载行为）
...
- 测试通道：`routeMock(pattern, body, {headers: {"Content-Disposition": "attachment; filename=x.txt"}})` 触发真下载
