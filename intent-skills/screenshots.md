...
- 陷阱：full 走超视口拼接会触发重布局（resize），用户在看的流程中途别用，改视口截图；fixed/sticky 页头拼接图像里可能重复出现；高 DPI 下截图是设备像素，从图上目测的坐标要除 devicePixelRatio 才是 CSS 点击坐标；导航后紧接截图的提交窗竞速 host 层已统一等提交，Not attached 有界重试内建
